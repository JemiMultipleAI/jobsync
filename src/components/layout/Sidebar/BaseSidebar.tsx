...
export interface BaseSidebarProps {
  ...
  basePath?: string; // Base path for active route detection (e.g., "/admin", "/user", "/employer")
  className?: string;
}
...
export function BaseSidebar({
  ...
  // Detect mobile screen size - only after mount to avoid hydration mismatch
  useEffect(() => {
    setMounted(true);
    const checkMobile = () => {
  ...
  return (
    <motion.aside
      ...
      className={cn(
        "fixed left-0 top-0 z-40 h-screen border-r border-border/50 backdrop-blur-md bg-background/80 shadow-xl overflow-hidden",
        className
      )}
    >
      <div className="flex h-full flex-col">
          ...
          {isOpen ? (
            ...
            <>
              ...
              <div className="flex w-full items-center justify-between gap-2 px-2">
                ...
                {/* Button only shows on desktop when closed - only render after mount to avoid hydration mismatch */}
                {mounted && !isMobile && (
                  <Button
                    variant="ghost"
                    size="icon"
        ...
        <nav className="flex-1 space-y-1 overflow-y-auto px-2 py-4">
          {navigation.map((item) => {
            // Dashboard should only be active on exact base path route
            ...
            <AnimatePresence mode="wait">
              {isOpen ? (
                ...
                >
                  ...
                  {footerSubtext && <p className="mt-0.5 text-[10px]">{footerSubtext}</p>}
                </motion.div>
              ) : (
                <motion.div
                ...
                >
                  <p className="text-[9px] font-medium text-muted-foreground">V1.0</p>
                </motion.div>
              )}
            </AnimatePresence>
          </div>
