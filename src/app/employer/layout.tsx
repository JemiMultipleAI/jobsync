...
export default function EmployerLayout({
  ...
  useEffect(() => {
    const handleResize = () => {
      ...
      if (isMobile) {
        ...
        const wasOpen =
          sessionStorage.getItem("employerSidebarOpen") === "true";
        setIsSidebarOpen(wasOpen);
      }
    };

    // Check initial screen size - only after mount
    const isMobile = window.innerWidth < MOBILE_BREAKPOINT;
    if (!isMobile) {
      // On desktop, check if sidebar was open before
      const wasOpen = sessionStorage.getItem("employerSidebarOpen") === "true";
  ...
  useEffect(() => {
    if (window.innerWidth >= MOBILE_BREAKPOINT) {
      sessionStorage.setItem("employerSidebarOpen", isSidebarOpen.toString());
    }
  }, [isSidebarOpen]);
