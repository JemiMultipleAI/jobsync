...
export default function Footer() {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      try {
        const response = await apiClient.get<{ user: { role: string } }>("/api/auth/profile");
  ...
  const handleEmployerLink = (e: React.MouseEvent<HTMLAnchorElement>, href: string) => {
    if (userRole !== "employer") {
      e.preventDefault();
      router.push(`/auth/login?redirect=${encodeURIComponent(href)}&role=employer`);
    }
  };
  return (
          ...
          <div>
            ...
            <ul className="space-y-3">
              <li>
                <Link 
                  href="/employer/jobs" 
                  onClick={(e) => handleEmployerLink(e, "/employer/jobs")}
                  className="text-gray-300 hover:text-[#B260E6] transition-colors flex items-center group"
                >
                  <ArrowRight className="h-3 w-3 mr-2 text-[#ED84A5] group-hover:text-[#B260E6] transition-colors" />
                  Post a Job
                </Link>
              ...
              <li>
                <Link 
                  ...
                  onClick={(e) => handleEmployerLink(e, "/employer/browse-workers")}
                  className="text-gray-300 hover:text-[#B260E6] transition-colors flex items-center group"
                >
                  <ArrowRight className="h-3 w-3 mr-2 text-[#ED84A5] group-hover:text-[#B260E6] transition-colors" />
                  Find Workers
              ...
              <li>
                <Link 
                  href="/employer" 
                  onClick={(e) => handleEmployerLink(e, "/employer")}
                  className="text-gray-300 hover:text-[#B260E6] transition-colors flex items-center group"
                >
                  <ArrowRight className="h-3 w-3 mr-2 text-[#ED84A5] group-hover:text-[#B260E6] transition-colors" />
                  Employer Dashboard
              ...
              <li>
                <Link 
                  ...
                  onClick={(e) => handleEmployerLink(e, "/employer/company")}
                  className="text-gray-300 hover:text-[#B260E6] transition-colors flex items-center group"
                >
                  <ArrowRight className="h-3 w-3 mr-2 text-[#ED84A5] group-hover:text-[#B260E6] transition-colors" />
                  Company Profile
